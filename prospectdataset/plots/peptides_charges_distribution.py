...
def main(sys_args=sys.argv[1:]):
    ...
    df_pie = meta_data.groupby(['precursor_charge', 'peptide_length']).size().unstack(fill_value=0)
    colors = sns.color_palette()[0:6]

    f, axes = plt.subplots(1,len(meta_data["peptide_length"].unique()), figsize=(20,5))
    for ax, col in zip(axes, df_pie.columns):
        patches, text = ax.pie(df_pie[col].values, colors=colors)
        ax.set(title=col)

    plt.legend(labels=['1', '2', '3', '4', '5', '6'], loc='center', ncol=6, bbox_to_anchor=(0.5, 2),
